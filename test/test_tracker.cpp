...
int main() {
    ...
    std::string im_path =
        "/home/zhaoran/thesis_ws/mpl/test_data/tum.1.photo.png";
    cv::Mat im = cv::imread(im_path);
    cv::Mat im_clone = im.clone();
    cv::cvtColor(im, im, CV_BGR2GRAY);

    std::string im2_path =
        "/home/zhaoran/thesis_ws/mpl/test_data/tum.2.photo.png";
    ...
    cv::Mat depth1 =
        cv::imread("/home/zhaoran/thesis_ws/mpl/test_data/tum.1.depth.png");
    ...
    for (int lvl = 0; lvl < lvls; ++lvl) {
        for (int i = 0; i < candidates.size(); ++i) {
            ...
            if (depth_pyramid[lvl].at<ushort>(candidates[i](1) >> 2,
                                              candidates[i](0) >> 2) == 0) {
                continue;
            }
            Eigen::Vector2i point(
                candidates[i](0) / (lvl + 1) + 1.f / (lvl + 1),
                candidates[i](1) / (lvl + 1) + 1.f / (lvl + 1));
            Eigen::Vector3f p3d = ref->unproject(
                point,
                depth_pyramid[lvl].at<ushort>(candidates[i](1) >> 2,
                                              candidates[i](0) >> 2) /
                    1000.f,
                lvl);
            // todo make it better
            (*pcp)[lvl].push_back(
                Voxel(p3d, ref->getImagePyramid()->operator()(
                               lvl, candidates[i](0), candidates[i](1))));
        }
    }
    ...
    for (int i = 0; i < lvls; ++i) {
        cv::Mat tmp;
        cv::resize(im2_clone, tmp, cv::Size(), 1.0f / (i + 1), 1.0f / (i + 1));
