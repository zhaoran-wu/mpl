...
int main() {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        if (sw.empty()) {
            curr_frame->set_pose(Sophus::SE3f(T_w_c0.rotation(), T_w_c0.translation()));
        ...
        Sophus::SE3f in_out_T_curr_lastKF = Sophus::SE3f(Eigen::Quaternionf::Identity(), Eigen::Vector3f::Zero());
        AffineLight in_out_aff_light_curr_lastKF(0, 0);

        tracker.tracking(curr_frame, in_out_T_curr_lastKF, in_out_aff_light_curr_lastKF);
        ...
        for (auto& pcd : pcp->operator[](0)) {
            ...
            // depth,
            cv::circle(img_draw_vec, cv::Point2f(hit_pixel(0), hit_pixel(1)), 2, cv::Scalar(0, 255, 0), 2);
        }
