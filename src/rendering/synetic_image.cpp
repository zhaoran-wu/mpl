...
void SyneticImage::preComputeParam() {
    ...
    extrinsic_T_c_cgl =
        Eigen::Isometry3f::Identity();  // cgl: opengl gl camera frame
...
cv::Mat SyneticImage::renderingAt(const Eigen::Isometry3f& pose,
    ...
    if (mode == mpl::DEPTH) {
        ...
        glBufferData(GL_PIXEL_PACK_BUFFER,
                     camera.cols * camera.rows * 4 * sizeof(ushort), 0,
                     GL_STREAM_READ);
        glReadBuffer(GL_BACK);
        glReadPixels(0, 0, camera.cols, camera.rows, GL_RGBA, GL_UNSIGNED_SHORT,
                     0);
        ...
        std::for_each(result_single_channnel.begin<ushort>(),
                      result_single_channnel.end<ushort>(), [=](ushort& depth) {
                          depth *= (1.f / std::numeric_limits<ushort>::max()) *
                                   1000 * z_f;
                      });
        ...
        glBindBuffer(GL_PIXEL_PACK_BUFFER, pbo);
        glBufferData(GL_PIXEL_PACK_BUFFER,
                     camera.cols * camera.rows * 4 * sizeof(uchar), 0,
                     GL_STREAM_READ);
        glReadBuffer(GL_BACK);
...
inline void SyneticImage::rendering_normal_image(
...
std::vector<cv::Mat> SyneticImage::renderingAt(const Eigen::Isometry3f& pose) {
    ...
    std::for_each(
        depth_single_channnel.begin<ushort>(),
        depth_single_channnel.end<ushort>(), [=](ushort& depth) {
            depth *= (1.f * 1000 * z_f / std::numeric_limits<ushort>::max());
        });
