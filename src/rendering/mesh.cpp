...
void Mesh::loadTextureImage(const std::string texture_file_path) {
  ...
  if (im_bgr.data) {
    ...
    glBindTexture(GL_TEXTURE_2D, texture_id);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, im_bgr.cols, im_bgr.rows, 0, GL_BGR,
                 GL_UNSIGNED_BYTE, im_bgr.data);
...
void Mesh::uploadToGpu() {
  ...
  glBufferData(GL_ARRAY_BUFFER, ply_file.vertices.size() * sizeof(Vertex),
               &ply_file.vertices[0], GL_STATIC_DRAW);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               ply_file.indices.size() * sizeof(unsigned int),
               &ply_file.indices[0], GL_STATIC_DRAW);
